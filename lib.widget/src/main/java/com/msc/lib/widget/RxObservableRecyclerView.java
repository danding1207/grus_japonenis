...
/**
 * Created by msc on 2016/4/5.
 */
public class RxObservableRecyclerView extends RecyclerView implements Scrollable {
    ...
    public RxObservableRecyclerView(Context context) {
        super(context);
        init();
    }

    public RxObservableRecyclerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public RxObservableRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    @CheckResult
    @NonNull
    public static Observable<OnObservableScrollViewCallbacksEvent> setOnObservableScrollViewCallbacksEvent(@NonNull RxObservableRecyclerView view) {
        return Observable.create(new ViewSelectOnSubscribe(view));
    }

    public static class OnObservableScrollViewCallbacksEvent {
        ...
        public OnObservableScrollViewCallbacksEvent(int type, int scrollY, boolean firstScroll, boolean dragging, ScrollState scrollState) {
    ...
    public static class ViewSelectOnSubscribe implements Observable.OnSubscribe<OnObservableScrollViewCallbacksEvent> {
        private final RxObservableRecyclerView view;

        public ViewSelectOnSubscribe(RxObservableRecyclerView view) {
            this.view = view;
        }

        @Override
        public void call(final Subscriber<? super OnObservableScrollViewCallbacksEvent> subscriber) {
            checkUiThread();
            ...
            ObservableScrollViewCallbacks listener = new ObservableScrollViewCallbacks() {
                ...
                public void onScrollChanged(int scrollY, boolean firstScroll, boolean dragging) {
                    if (!subscriber.isUnsubscribed()) {
                        subscriber.onNext(new OnObservableScrollViewCallbacksEvent(1, scrollY, firstScroll, dragging, null));
                    }
                }
                ...
                public void onDownMotionEvent() {
                    if (!subscriber.isUnsubscribed()) {
                        subscriber.onNext(new OnObservableScrollViewCallbacksEvent(2, 0, false, false, null));
                    }
                }
                ...
                public void onUpOrCancelMotionEvent(ScrollState scrollState) {
                    if (!subscriber.isUnsubscribed()) {
                        subscriber.onNext(new OnObservableScrollViewCallbacksEvent(3, 0, false, false, scrollState));
                    }
                }
            };
            view.setScrollViewCallbacks(listener);

            subscriber.add(new MainThreadSubscription() {
                @Override
                protected void onUnsubscribe() {
                    view.setScrollViewCallbacks(null);
                }
            });
        }
